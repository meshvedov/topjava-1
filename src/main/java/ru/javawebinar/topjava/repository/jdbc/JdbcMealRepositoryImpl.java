...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public Meal get(int id, int userId) {
        List<Meal> meal = jdbcTemplate.query("SELECT * FROM meals WHERE userId = ? AND id = ?", ROW_MAPPER, userId, id);
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE userid=? AND datetime >= ? AND datetime <= ?", ROW_MAPPER, userId, startDate, endDate);
    }
}
