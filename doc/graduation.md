...
### Отнеситесь к проекту как к вашему тестовому заданию на работу! Это означает, что у НЕ будет возможности объяснять, почему ты так сделал код и что для запуска надо создать базу данных и что-то еще. Если что-то не по ТЗ или не запускается или код странный без объяснений - молча вычеркнут из кандидатов.
